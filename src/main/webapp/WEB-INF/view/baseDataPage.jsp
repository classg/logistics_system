...
<div class="page-container">
    <span id="toolbar" class="l">
	 	...
	 	<a href="javascript:;" onclick="baseData_add()" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a>
	 </span>
...
<script type="text/javascript">
    ...
    $(function() {
        $('#userTable').bootstrapTable({
            url: 'baseData/list.do',// 发送过来的ajax数据的地址
            /**
             * ajax请求以后回调函数的处理
    ...
    function operationFormatter(value,row,index){
        ...
        var html = "";
        html += "<span onclick='baseData_edit("+value+")' style='color: green; cursor: pointer' class='glyphicon glyphicon-pencil' />";
        html += "<span onclick='baseData_del("+value+")' style='color: red; margin-left: 30px; cursor: pointer' class='glyphicon glyphicon-trash' />";
        return html;
    }

    function baseDataType(value,row,index){
        var html = "";
        if(value == "baseData"){
    ...
    */
    ...
    function baseData_add(){
        layer_show("新增管理员","baseData/edit.do",800,500);
    }
    /*管理员-编辑*/
    function baseData_edit(baseDataId){
        layer.open({
            type: 2,
            content: "baseData/edit.do?baseId="+baseDataId,
            title: "更新管理员",
            area: ["800px","500px"]
        })
    }
    ...
    function baseData_del(baseId){
        layer.confirm('亲，请问您确认要删除吗',function(index){
            $.post("baseData/baseData_del.do",{"baseId":baseId},function(data){
                // 弹出一个提示框("内容"，时间等)
                console.log(baseId+"-----");
